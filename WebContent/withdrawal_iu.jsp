...
<%
	...
	try {
		...
		int withdrawal = Integer.parseInt(request.getParameter("money"));
		Class.forName("com.mysql.jdbc.Driver");
		Connection conn = DriverManager.getConnection(url, uid, pass);
		PreparedStatement stmt = conn.prepareStatement(sql);
		
		stmt.setString(1, id);
		stmt.setInt(2, withdrawal);
...
%>
<script>
	alert("<%=withdrawal%>원이 출금 완료 되었습니다. 거래내역 조회 화면으로 이동합니다.");
	location.href="mylist.jsp";
</script>
	<% } catch (Exception e) {
		out.print("출금 진행 중에 오류가 발생했습니다. 고객센터로 문의주세요.<p>" + e.getMessage());
	}
%>
